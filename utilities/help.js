...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (img == null) img = "https://cdn.discordapp.com/attachments/1094261264454254672/1115348419351937064/fallout-vault-boy.gif"
            ...
            if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
                ...
                message.channel.send({ embeds: [embed], components: [helpset] }).then(async msg => {
                    ...
                    collector.on("collect", async (c) => {
                        ...
                        if (value === "menu") {
                            db.set(`help`, "help")
                            c.reply({ content: `Votre help sera désormais affiché sous forme de **menu**`, ephemeral: true })
                            msg.edit({ embeds: [menu] })
                        }

                        else if (value === "button") {
                            db.set(`help`, "helpb")
                            c.reply({ content: `Votre help sera désormais affiché avec des **boutons**`, ephemeral: true })
        ...
        if (args[0] === "all") {
            ...
            const imgperm = "https://cdn.discordapp.com/attachments/1094261264454254672/1115348419351937064/fallout-vault-boy.gif"
            ...
            const help = new Discord.MessageEmbed()
                ...
                .setDescription(`・Ce bot appartient à <@${config.app.owners}>`)
                .setImage(imgperm)
                .setColor(color)
...
\`\`\`
    ...
    **\`${pf}drop\`**
    Permet de lancer un concour rapide
...
Salon qui sera utilisé pour envoyé toutes les suggestions proposés par les membres
                ...
                .setDescription(`
                    ...
                    **\`${pf}giveaway <salon> <temps> <nombre winners> <Gain>\`**
                    Permet de lancer un Giveaway sur le serveur
                    **\`${pf}drop\`**
                    Permet de lancer un concours rapide
                `)
                                .setColor(color);
